...
public:	
	...
	void setContent(std::vector<Point>& v, int content);		
	...
	bool areAllLocsEmpty(const std::vector<Point>& v) const;  // just changed the parameter to non reference???
...
private:  // This is commented out to allow us to test. 
	...
	// - return: nothing
	void removeRows( std::vector<int>& v);
};
